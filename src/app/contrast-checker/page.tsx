...
import { Check, X, AlertTriangle, Contrast, RefreshCw, Lightbulb, Type, Eye, ArrowRight, Info } from 'lucide-react';
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-6 sm:py-8 sm:px-6 lg:px-8">
      ...
      <div className="mb-6 sm:mb-8">
        <p className="text-xs font-medium text-gray-500 mb-2 sm:mb-3">Quick Presets:</p>
        <div className="flex gap-2 overflow-x-auto pb-2 -mx-4 px-4 sm:mx-0 sm:px-0 sm:flex-wrap scrollbar-hide">
          {colorPresets.map((preset) => (
            <button
              key={preset.name}
              onClick={() => applyPreset(preset)}
              className="flex-shrink-0 flex items-center gap-2 px-3 py-2 rounded-xl bg-white/5 border border-white/10 hover:border-violet-500/30 transition-all active:scale-[0.98]"
            >
              ...
              <span className="text-xs sm:text-sm text-gray-300">{preset.name}</span>
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="grid gap-4 sm:gap-6 lg:gap-8 lg:grid-cols-3">
        {/* Color inputs */}
        <div className="bg-gray-900/50 backdrop-blur border border-white/10 rounded-2xl p-4 sm:p-5 lg:col-span-1">
          <h3 className="text-sm font-semibold text-white mb-1">Colors</h3>
          <p className="text-xs text-gray-500 mb-4">Enter foreground and background</p>
          ...
          <div className="space-y-3 sm:space-y-4">
            ...
            <div>
              ...
              <div className="flex gap-2">
                ...
                <input
                  ...
                  value={foreground}
                  onChange={(e) => handleForegroundChange(e.target.value)}
                  className="h-10 flex-1 rounded-lg border border-white/10 bg-black/20 px-3 font-mono text-sm text-white focus:border-violet-500 focus:outline-none"
                  placeholder="#000000"
                />
              </div>
            </div>
            ...
            <div className="flex justify-center">
              <button
                onClick={swapColors}
                className="rounded-lg bg-white/5 border border-white/10 px-4 py-2 text-xs font-medium text-gray-300 hover:bg-white/10 transition-colors flex items-center gap-2"
              >
            ...
            <div className="mt-4 pt-4 border-t border-white/5">
              ...
              <pre className="text-[10px] sm:text-xs bg-black/20 border border-white/5 rounded-lg p-2.5 overflow-x-auto font-mono text-gray-400">
                {cssSnippet}
              </pre>
            </div>
          </div>
        ...
        {/* Results */}
        <div className="bg-gray-900/50 backdrop-blur border border-white/10 rounded-2xl p-4 sm:p-5 lg:col-span-2">
          <h3 className="text-sm font-semibold text-white mb-1">Contrast Analysis</h3>
          <p className="text-xs text-gray-500 mb-4">WCAG 2.1 compliance</p>
          
          {result && (
            <div className="space-y-4 sm:space-y-6">
              {/* Main score - Stack on mobile */}
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 p-4 sm:p-6 rounded-xl bg-black/20 border border-white/5">
                <div>
              ...
              <div>
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className="text-sm font-medium text-gray-700 dark:text-gray-300">Size Comparison</span>
                </div>
              ...
              <div>
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className="text-sm font-medium text-gray-700 dark:text-gray-300">WCAG Compliance</span>
                </div>
                ...
                <div className="grid gap-2 sm:grid-cols-2">
                  {[
                    ...
                    >
                      ...
                      <span className="text-xs font-mono text-gray-500">{item.requirement}</span>
                    </div>
                  ))}
                </div>
              </div>
      ...
      {result && result.score !== 'AAA' && suggestions && (
        <div className="mt-6 sm:mt-8 bg-gray-900/50 backdrop-blur border border-white/10 rounded-2xl p-4 sm:p-6">
          <div className="flex items-center gap-2 mb-1">
            <Lightbulb className="h-4 w-4 sm:h-5 sm:w-5 text-amber-500" />
            <h3 className="text-sm sm:text-base font-semibold text-white">Suggested Improvements</h3>
          </div>
          <p className="text-xs text-gray-500 mb-4">Alternative colors that meet AA or AAA standards</p>
          
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {suggestions.foreground.length > 0 && (
              <div className="space-y-3">
                <p className="text-xs sm:text-sm font-medium text-gray-300">Adjust Text Color</p>
                  ...
                  {suggestions.foreground.slice(0, 3).map((color, i) => {
                    ...
                    return (
                      <button
                        ...
                        className="flex items-center gap-3 w-full p-3 rounded-lg bg-black/20 border border-white/10 hover:border-violet-500/30 transition-all active:scale-[0.98] group"
                      >
                        <span 
                          className="w-7 h-7 sm:w-8 sm:h-8 rounded-lg ring-1 ring-white/10"
                          style={{ backgroundColor: color }}
                        />
                        <div className="text-left flex-1">
                          <p className="font-mono text-xs sm:text-sm text-white">{color}</p>
                          <p className="text-xs text-gray-500">{newResult.ratioString} - {newResult.score}</p>
                          </div>
                        <ArrowRight className="h-4 w-4 text-gray-400 group-hover:text-violet-500 transition-colors" />
                      </button>
                    );
                  })}
                </div>
            ...
            {suggestions.background.length > 0 && (
              <div className="space-y-3">
                <p className="text-xs sm:text-sm font-medium text-gray-300">Adjust Background</p>
                <div className="space-y-2">
      ...
      <div className="mt-6 sm:mt-8 bg-gray-900/50 backdrop-blur border border-white/10 rounded-2xl p-4 sm:p-6">
        <div className="flex flex-col sm:flex-row sm:items-start gap-3 sm:gap-4">
          ...
          <div className="flex-1">
            <h4 className="font-semibold text-white text-sm sm:text-base mb-2">Understanding WCAG Contrast</h4>
            ...
            <div className="grid sm:grid-cols-2 gap-3 sm:gap-4">
              <div className="p-3 sm:p-4 rounded-lg bg-black/20 border border-white/5">
                <h5 className="font-medium text-white text-xs sm:text-sm mb-2">Text Sizes</h5>
                <ul className="space-y-1 text-xs text-gray-400">
                  <li><strong className="text-gray-300">Normal text:</strong> Under 18pt or under 14pt bold</li>
                  <li><strong className="text-gray-300">Large text:</strong> 18pt+ or 14pt+ bold</li>
                </ul>
              </div>
              <div className="p-3 sm:p-4 rounded-lg bg-black/20 border border-white/5">
                <h5 className="font-medium text-white text-xs sm:text-sm mb-2">WCAG Levels</h5>
                <ul className="space-y-1 text-xs text-gray-400">
                  <li><strong className="text-gray-300">Level AA:</strong> Minimum acceptable contrast</li>
                  <li><strong className="text-gray-300">Level AAA:</strong> Enhanced, recommended</li>
                </ul>
              </div>
            </div>
          </div>
