...
import { Search, ArrowRightLeft, Lightbulb, Sparkles, Palette, ArrowRight, Info, Eye } from 'lucide-react';
    ...
    <div className="mx-auto max-w-5xl px-4 py-4 sm:py-8 sm:px-6 lg:px-8">
      ...
      <div className="mx-auto mb-4 sm:mb-8 max-w-2xl">
        <div className="flex flex-col sm:flex-row gap-2">
          <div className="relative flex-1">
            <Search className="absolute left-3 sm:left-4 top-1/2 h-4 w-4 sm:h-5 sm:w-5 -translate-y-1/2 text-gray-500" />
            <input
              type="text"
              value={inputValue}
              ...
              className="h-12 sm:h-14 w-full rounded-xl border border-white/10 bg-gray-900/50 backdrop-blur pl-10 sm:pl-12 pr-4 text-sm sm:text-base text-white placeholder:text-gray-500 focus:border-violet-500 focus:outline-none focus:ring-1 focus:ring-violet-500"
            />
          </div>
          <button
            onClick={handleLookup}
            className="w-full sm:w-auto rounded-xl bg-violet-600 px-6 sm:px-8 py-3 font-semibold text-sm sm:text-base text-white transition-all hover:bg-violet-500 active:scale-[0.98] shadow-lg shadow-violet-500/25"
          >
            Lookup
          </button>
        </div>
        ...
        {!colorData && (
          <div className="mt-4 sm:mt-6">
            <p className="text-xs sm:text-sm text-gray-500 mb-2 sm:mb-3">Try these popular colors:</p>
            <div className="flex flex-wrap gap-1.5 sm:gap-2">
              {popularColors.map((color) => (
                <button
                  key={color.name}
                  onClick={() => handleQuickColor(color.value)}
                  className="flex items-center gap-1.5 sm:gap-2 px-2.5 sm:px-3 py-1.5 sm:py-2 rounded-lg border border-white/10 hover:border-violet-500/50 active:scale-[0.98] transition-all bg-gray-900/50"
                >
                  <span 
                    className="w-3.5 h-3.5 sm:w-4 sm:h-4 rounded-full ring-1 ring-white/20"
                    style={{ backgroundColor: color.value }}
                  />
                  <span className="text-xs sm:text-sm text-gray-300">{color.name}</span>
                </button>
              ))}
            </div>
          </div>
      ...
      {colorData && (
        <div className="space-y-4 sm:space-y-6">
          {/* Main color display */}
          <div className="bg-gray-900/50 backdrop-blur border border-white/10 rounded-xl sm:rounded-2xl overflow-hidden">
            <div
            ...
            <div className="p-3 sm:p-4">
              ...
              <div className="grid gap-1.5 sm:gap-2 grid-cols-2 lg:grid-cols-4">
                {[
                  ...
                  <div
                    key={format.label}
                    className="flex items-center justify-between rounded-lg bg-black/20 border border-white/5 px-2.5 sm:px-3 py-1.5 sm:py-2"
                  >
                    <div className="min-w-0 flex-1">
                      <span className="text-[8px] sm:text-[10px] font-semibold uppercase text-gray-500">
                        {format.label}
                      </span>
                      <p className="font-mono text-[10px] sm:text-xs text-gray-200 truncate">
                        {format.value}
                      </p>
                    </div>
              ...
              <Link 
                ...
                className="flex items-center justify-between w-full p-2.5 sm:p-3 mt-2 sm:mt-3 rounded-xl bg-violet-500/10 border border-violet-500/20 hover:bg-violet-500/20 active:scale-[0.99] transition-all group"
              >
                <div className="flex items-center gap-2 sm:gap-3">
                  ...
                  <div>
                    <p className="text-xs sm:text-sm font-medium text-violet-300">Explore in Generator</p>
                    <p className="text-[10px] sm:text-xs text-violet-400/70">Shades, tints, harmonies</p>
                  </div>
                </div>
                <ArrowRight className="h-3.5 w-3.5 sm:h-4 sm:w-4 text-violet-400 group-hover:translate-x-1 transition-transform" />
              </Link>
            </div>
          </div>
          ...
          <div className="grid gap-3 sm:gap-4 lg:grid-cols-2">
            {/* Color Psychology */}
            {colorMeaning && (
              <div className="bg-gray-900/50 backdrop-blur border border-white/10 rounded-xl sm:rounded-2xl p-3 sm:p-5">
                <div className="flex items-center gap-2 mb-3 sm:mb-4">
                  ...
                  <div>
                    <h3 className="font-semibold text-sm sm:text-base text-white">Color Psychology</h3>
                    <p className="text-[10px] sm:text-xs text-gray-500">How this color is perceived</p>
                  </div>
                </div>
                <div className="space-y-3 sm:space-y-4">
                  <div>
                    <p className="text-[8px] sm:text-[10px] font-semibold uppercase text-gray-500 mb-0.5 sm:mb-1">Mood & Feel</p>
                    <p className="text-xs sm:text-sm text-gray-200">{colorMeaning.mood}</p>
                  </div>
                  <div>
                    <p className="text-[8px] sm:text-[10px] font-semibold uppercase text-gray-500 mb-1 sm:mb-2">Best Used For</p>
                    <div className="flex flex-wrap gap-1 sm:gap-1.5">
                      {colorMeaning.usage.map((use, i) => (
                        <span key={i} className="text-[10px] sm:text-xs px-2 sm:px-2.5 py-0.5 sm:py-1 rounded-full bg-white/5 border border-white/10 text-gray-300">
                          {use}
                        </span>
                      ))}
                    </div>
                  ...
                  <div>
                    <p className="text-[8px] sm:text-[10px] font-semibold uppercase text-gray-500 mb-1 sm:mb-2">Popular Industries</p>
                    <div className="flex flex-wrap gap-1 sm:gap-1.5">
                      {colorMeaning.industries.map((ind, i) => (
                        <span key={i} className="text-[10px] sm:text-xs px-2 sm:px-2.5 py-0.5 sm:py-1 rounded-full bg-violet-500/10 border border-violet-500/20 text-violet-300">
                          {ind}
                        </span>
                      ))}
                    </div>
            ...
            {accessibility && (
              <div className="bg-gray-900/50 backdrop-blur border border-white/10 rounded-xl sm:rounded-2xl p-3 sm:p-5">
                <div className="flex items-center gap-2 mb-3 sm:mb-4">
                  ...
                  <div>
                    <h3 className="font-semibold text-sm sm:text-base text-white">Accessibility</h3>
                    <p className="text-[10px] sm:text-xs text-gray-500">WCAG contrast check</p>
                  </div>
                </div>
                <div className="space-y-2 sm:space-y-3">
                  <div className="p-2 sm:p-3 rounded-lg bg-black/20 border border-white/5">
                  ...
                  <div className="grid grid-cols-2 gap-2 sm:gap-3">
                    <div className="p-2 sm:p-3 rounded-lg bg-black/20 border border-white/5">
                      <div className="flex justify-between items-start mb-0.5 sm:mb-1">
                        <span className="text-[8px] sm:text-[10px] font-medium text-gray-500">White Text</span>
                        <span className={cn(
                    ...
                    <div className="p-2 sm:p-3 rounded-lg bg-black/20 border border-white/5">
                      <div className="flex justify-between items-start mb-0.5 sm:mb-1">
                        <span className="text-[8px] sm:text-[10px] font-medium text-gray-500">Black Text</span>
                        <span className={cn(
                          "text-[8px] sm:text-[10px] font-bold px-1 sm:px-1.5 py-0.5 rounded",
          ...
          {/* Color harmonies */}
          <div>
            <h3 className="text-sm sm:text-base font-semibold text-white mb-3 sm:mb-4 flex items-center gap-1.5 sm:gap-2">
              <Palette className="h-3.5 w-3.5 sm:h-4 sm:w-4 text-violet-400" />
              Color Harmonies
            </h3>
            <div className="grid gap-2 sm:gap-3 grid-cols-2 lg:grid-cols-3">
              {/* Complementary */}
              <div className="bg-gray-900/50 backdrop-blur border border-white/10 rounded-lg sm:rounded-xl p-2.5 sm:p-4 hover:border-white/20 transition-colors">
                <div className="mb-2 sm:mb-3">
                  <h4 className="text-xs sm:text-sm font-medium text-white">Complementary</h4>
                  <p className="text-[10px] sm:text-xs text-gray-500">Opposite on wheel</p>
                </div>
              ...
              <div className="bg-gray-900/50 backdrop-blur border border-white/10 rounded-lg sm:rounded-xl p-2.5 sm:p-4 hover:border-white/20 transition-colors">
                <div className="mb-2 sm:mb-3">
                  <h4 className="text-xs sm:text-sm font-medium text-white">Triadic</h4>
                  <p className="text-[10px] sm:text-xs text-gray-500">Three evenly spaced</p>
                </div>
              ...
              <div className="bg-gray-900/50 backdrop-blur border border-white/10 rounded-lg sm:rounded-xl p-2.5 sm:p-4 hover:border-white/20 transition-colors">
                <div className="mb-2 sm:mb-3">
                  <h4 className="text-xs sm:text-sm font-medium text-white">Analogous</h4>
                  <p className="text-[10px] sm:text-xs text-gray-500">Adjacent colors</p>
                </div>
                <div className="flex gap-1 sm:gap-2">
              ...
              <div className="bg-gray-900/50 backdrop-blur border border-white/10 rounded-lg sm:rounded-xl p-2.5 sm:p-4 hover:border-white/20 transition-colors">
                <div className="mb-2 sm:mb-3">
                  <h4 className="text-xs sm:text-sm font-medium text-white">Split Comp.</h4>
                  <p className="text-[10px] sm:text-xs text-gray-500">Adjacent to complement</p>
                </div>
                <div className="flex gap-1 sm:gap-2">
              ...
              {/* Tetradic */}
              <div className="bg-gray-900/50 backdrop-blur border border-white/10 rounded-lg sm:rounded-xl p-2.5 sm:p-4 hover:border-white/20 transition-colors col-span-2 lg:col-span-1">
                <div className="mb-2 sm:mb-3">
                  <h4 className="text-xs sm:text-sm font-medium text-white">Tetradic</h4>
                  <p className="text-[10px] sm:text-xs text-gray-500">Four in rectangle</p>
                </div>
                <div className="flex gap-1 sm:gap-2">
